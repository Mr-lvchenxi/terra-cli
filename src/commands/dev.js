...
import { getAccessToken, getUserInfo, getUserNpm, hasPermission } from '../utils/git'
async function devAction (port) {
    ...
    try {
    	// npm 无对应包时会报错 500
			...
			if (!result.flag) {
				console.log(chalk.green(`您当前没有${componentInfo.name}组件的开发权限，申请权限请联系${result.admin}老师, 工号${result.workCode}`))
				return
			}
		} catch (e) {
