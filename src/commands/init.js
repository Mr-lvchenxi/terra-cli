...
async function getWidgetDir(targetPath) {
  ...
  const baseName = aValidNameFromPath
    ? parsedTargetPath.base
    : await getText("输入项目名称: ( 如: testComponent, 小驼峰 )");
...
async function initAction(targetPath = "") {
  ...
  await fs.copy(
    COMMON_TEMPLATE_DIR + "/package.json",
    widgetDir + "/package.json"
  );
  await fs.copy(
    COMMON_TEMPLATE_DIR + "/.gitignoreTemplate",
    widgetDir + "/.gitignore"
  );
  await fs.copy(
    PROJECT_TEMPLATE_PATH_CONFIG[packageJsonConfig.componentType],
    widgetDir
  );

  // git 相关
  // const gitName = formatComponentName(
  //   `${packageJsonConfig.department}/${componentName}`
  // );
  ...
  await renderDir(widgetDir, {
    ...
    componentName: str(targetPath || componentName)
      .capitalize()
      .value(), // 驼峰格式：ComponentName
