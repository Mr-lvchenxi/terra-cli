...
const header = ['@thinkacademy','@monkey','@izk','@page','@neirongyun', '@hybrid', '@izk', '@PaaS', '@glo', '@qz', '@abk', '@DMP', '@leap','@udc','@peiyou','@xes', '@yachfe']
// 定义需要询问的问题
const questions = [
  {
    type: 'rawlist',
...
export async function getPackageJsonConfig(params = {}) {
    ...
    return new Promise(async (resolve, reject)=>{
			...
			if (res) {
				...
				console.log(chalk.red('该部门已存在相同类型，相同名称的组件，请修改名称'))
				resolve(false)
			}
    })
  }
...
export async function getComponentInfo() {
  ...
  return {
    ...
    department: sourcePackageContent.department,
		...
		demoUrl: `${BUCKET_BASE_URL}/${formatComponentPath(sourcePackageContent)}/index.html?v=${+new Date()}`,
		readmeUrl: `${BUCKET_BASE_URL}/${formatComponentPath(sourcePackageContent)}/README.md?v=${+new Date()}`,
...
export function formatCdnPath({name, version}) {
	return {
		minUrl: `${BUCKET_BASE_URL}/${formatComponentPath({name})}/lib/${version}/${formatComponentName(name)}.min.js`,
		url: `${BUCKET_BASE_URL}/${formatComponentPath({name})}/lib/${version}/${formatComponentName(name)}.js`
	}
}
/**
 * 同名组件校验
